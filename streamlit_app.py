...
# ═══════════════════════════════════════════════════════════════
_APP_VERSION = "v7.88"
_APP_BUILD   = "04 Aug 2026 20:24"   # auto-updated by pre-commit hook
...
with tab_silvermic:
    ...
    # Guard: need a Fyers token (use flag instead of st.stop() which kills the
    # entire Streamlit page, not just this tab)
    _sm_token = st.session_state.get("fyers_token", "")
    _sm_ready = bool(_sm_token)
    if not _sm_ready:
        ...
        if _rows:
            ...
            with st.expander("🔧 Diagnostics"):
                try:
                    ...
                    st.write("Tradeable (expiry > 11d):", _dbg["tradeable"] or "— none —")
